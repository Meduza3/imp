...
void run_machine(vector<pair<int, long long>> &program)
{
  ...
  while (program[lr].first != HALT) // HALT
  {
    ...
    case GET:
      cout << "? ";
      cin >> p[program[lr].second];
      io += 100;
      t += 100;
      lr++;
      break;
    case PUT:
      cout << "> " << p[program[lr].second] << endl;
      io += 100;
      t += 100;
      lr++;
      break;
    ...
    case LOAD:
      p[0] = p[program[lr].second];
      t += 10;
      lr++;
      break;
    case STORE:
      p[program[lr].second] = p[0];
      t += 10;
      lr++;
      break;
    case LOADI:
      p[0] = p[p[program[lr].second]];
      t += 20;
      lr++;
      break;
    case STOREI:
      p[p[program[lr].second]] = p[0];
      t += 20;
      lr++;
      break;
