...
def _run_assembler_generator_test(input_code: str, expected_output_numbers: list[int], user_input: str = "") -> None:
    ...
    vm_result = subprocess.run(
        [os.path.join(MAKEFILE_PATH, "resources/maszyna_wirtualna/maszyna-wirtualna"), OUTPUT_FILENAME], capture_output=True, text=True, input=user_input
    )

    big_numbers_vm_result = subprocess.run(
        [os.path.join(MAKEFILE_PATH, "resources/maszyna_wirtualna/maszyna-wirtualna-cln"), OUTPUT_FILENAME], capture_output=True, text=True, input=user_input
    )
